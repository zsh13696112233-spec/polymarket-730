...
开发和生产启动命令均监听 `0.0.0.0`（前端 `3000`、后端 `8730`）。内网访问时，在 `.env` 中将 `NEXT_PUBLIC_API_BASE` 设为 `http://服务器内网IP:8730`，将 `POLYMARKET_CORS_ORIGINS` 设为 `http://服务器内网IP:3000`，然后重启服务，通过 `http://服务器内网IP:3000` 访问。生产模式需重新构建前端；内网 IP 变化后需同步更新这两个配置。

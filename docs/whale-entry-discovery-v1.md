...
> 此文档记录旧版全市场复杂发现方案，现已停止作为实现依据。当前的新账号、24 小时大额买入及持仓确认口径见 [`whale-new-account-monitor.md`](whale-new-account-monitor.md)。历史跟买执行与资金记录部分仍保留。
...
**增量优化**：正常扫描将实际采集到的最新 `timestamp` 持久化到 `last_trade_cursor_at`，只向前推进；空响应保持原游标，滞后响应不按扫描时间推进，旧的重叠成交不使游标倒退。重启后恢复该游标；尚无游标时回退到已入库的最新成交时间。下一轮的 `start` 取 `max(window_start, last_trade_cursor_at − 120s)`，回退 120 秒用于捕获重叠区间内的乱序与延迟成交。这能把稳态下的成交采集压到 1–2 页。无游标或游标早于窗口时回溯全窗口；分页触顶仍按上述覆盖保护流程推进到本轮结束时间。晚于重叠区间才出现的乱序成交仍可能遗漏，本机制不保证任意延迟的历史成交均被补回。
...
**顶部工具条**：只保留钱包/地址/市场搜索、按持仓价值或最近买入排序、刷新数据和「我的跟单」入口。最后更新时间使用最近成功扫描时间；只有扫描失败时才展示错误提示。采集阈值和扫描周期继续由系统设置维护，不在持仓首页编辑。

设置页同时维护巨鲸跟单的默认买入金额和单笔买入上限。点击「跟单」时自动带入默认金额，用户仍可在收益预览弹窗中使用 20/50/100 USDC 快捷金额或输入任意不超过单笔上限的金额。

**钱包持仓卡片**。首页按钱包聚合，每张卡片展示钱包公开头像、名称、地址、注册日期、当前持仓估值和仍在持有的市场数量；没有头像或图片加载失败时使用默认占位。卡片内每行是该钱包的一个 outcome 持仓，只展示市场、方向、剩余份额、当前估值、买入均价、当前价、最近买入时间和跟单入口。完全退出的仓位默认不展示；同一钱包持有同一市场两侧时保留两行并标记「双向持仓」。

接口额外返回 `net_size`、`current_value_usdc` 与 `wallet_avatar_url`，首页使用真实剩余份额而非累计买入量。前端把市场响应扁平化后按 `proxy_wallet` 分组，钱包内持仓按 `current_value_usdc` 降序排列。

时间统一用北京时间显示，与现有页面一致。

**空状态**：没有仍在持有的巨鲸仓位时提示刷新数据；搜索无结果时提供清除搜索操作。
...
- 同一钱包在多个市场持仓时合并为一张钱包卡片，内部按当前估值排序；
- 展示剩余份额和当前估值，完全退出的持仓不出现；
- 搜索支持钱包名称、地址和市场名称；
- 双向持仓保留两行并展示风险提示；
- 跟单预览展示押中结算净利润和按当前盘口立即卖出的预估盈亏；
- 单边市场只渲染一栏；
- `hedged` 条目显示「疑似做市」标记；
...
| A5 | 同一钱包持有某市场 Yes 与 No | 钱包卡片保留两条持仓，并分别展示「双向持仓」提示 |
